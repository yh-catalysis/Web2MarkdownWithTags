...
import { extractFilename, DOCUMENT_DOWNLOAD_TIMEOUT } from "../lib/constants.js";
import { validateUrl } from "../lib/validate-url.js";
import { readBodyWithLimit } from "../lib/fetch-utils.js";
import { isPaidConversion, isImageConversionAllowed } from "../lib/image-gate.js";
...
export async function convertToMarkdown(
  ...
  try {
    ...
    const resolvedFilename = input.filename ?? extractFilename(input.url, contentType);

    const mimeType = contentType?.split(";")[0].trim() ?? "application/octet-stream";
    if (isPaidConversion(resolvedFilename, mimeType) && !isImageConversionAllowed(env)) {
      return {
        ok: false,
    ...
    const result = await convertViaAI(env, resolvedFilename, fileBytes, mimeType);
