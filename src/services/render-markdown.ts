...
export async function renderMarkdown(
  ...
  try {
    ...
    try {
      ...
      await page.goto(input.url, { waitUntil: "networkidle0", timeout: FETCH_TIMEOUT });

      if (input.waitForSelector) {
        await page.waitForSelector(input.waitForSelector, { timeout: WAIT_FOR_SELECTOR_TIMEOUT });
      }

      const html = await page.content();
      await browser.close();

      const result = await convertViaAI(env, "rendered.html", html, "text/html");
